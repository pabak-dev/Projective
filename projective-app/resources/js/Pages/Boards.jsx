...
import {
  Plus, List, Clock, CheckCircle, AlertTriangle,
  MessageSquare, Paperclip, X, Info, GitBranch,
  UserPlus, Edit, Trash2, User,
} from "lucide-react";
...
export default function Boards() {
  ...
  const [newProjectData, setNewProjectData] = useState({
    name: "", description: "", version_control_link: "",
  });
  ...
  const fetchTasks = async (projectId) => {
    ...
    try {
      ...
      if (res.data) {
        res.data.forEach((t) => {
          if (!t.assignedUser && t.assignee_id) {
            const u = [...users, ...allUsers].find(u => u.id === t.assignee_id);
            if (u) t.assignedUser = u;
            else t.assignedUser = { id: t.assignee_id, name: `User ${t.assignee_id}` };
          }
...
const handleDragEnd = async (result) => {
  ...
  const assignTask = async (userId) => {
    try {
      ...
      } else if (!updatedTask.assignee_id) {
                ...
                <div className="flex items-center space-x-2 mt-4 sm:mt-0">
                  ...
                  {isOwner && (
                    <>
                      ...
                      <button
                        onClick={() => {
                          setNewProjectData(selectedProject);
  ...
  />
      ...
      >
        <form onSubmit={handleUpdateProject} className="p-6">
          ...
          <div className="mt-6 space-y-4">
            <div>
              <InputLabel htmlFor="edit-name" value="Project Name" />
              <TextInput
                id="edit-name"
                className="mt-1 block w-full"
                value={newProjectData.name}
      ...
      >
        <div className="p-6">
          ...
          <p className="mt-2 text-sm text-gray-600">
            ...
            {selectedProject?.name}" project and all of its associated data.
          </p>
          <div className="mt-6 flex justify-end space-x-2">
            <SecondaryButton onClick={() => setConfirmDeleteModalOpen(false)}>
              Cancel
            </SecondaryButton>
            <DangerButton onClick={handleDeleteProject}>Delete</DangerButton>
          </div>
        </div>
      </Modal>
          ...
          <div className="p-6">
            ...
            {selectedProject.version_control_link && (
              <div className="mt-4">
                ...
                >
                  <GitBranch size={16} className="mr-2" /> Version Control
                </a>
              </div>
            )}
            <div className="mt-6 flex justify-end">
              <SecondaryButton onClick={() => setProjectDetailsModalOpen(false)}>
                Close
              </SecondaryButton>
            </div>
          </div>
            ...
            {selectedProject?.members.map((member) => (
              ...
              >
                <span className="text-sm font-medium">
                  {member.name}{" "}
                  {member.id === selectedProject.user_id && "(Owner)"}
                </span>
